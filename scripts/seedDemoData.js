...
async function main() {
  ...
  try {
    ...
    for (const row of DEMO_SCENARIOS) {
      ...
      if (row.fallbackUsed) {
        const fallbackEvent = await AuditEvent.findOne({ merchant: merchant._id, recoveryAction: action._id, type: 'AI_FALLBACK_USED' });
        if (!fallbackEvent) await AuditEvent.create({ merchant: merchant._id, payment: payment._id, recoveryCase: recoveryCase._id, recoveryAction: action._id, type: 'AI_FALLBACK_USED', actor: 'SYSTEM', reason: 'Development demo fallback scenario.' });
      }
    }
